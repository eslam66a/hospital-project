...
bool AddPatient( int HisSpecialization , string HisName , int HisStatis , int count[]) {
    if ( count[HisSpecialization] >= 5 ) return false;
...
void GetNextPatients(int HisSpec , int count[]) {
    if (count[HisSpec] == 0 ) cout <<"No Patients in specialization " << HisSpec << endl;
    else {
...
while (true)
    {
        ...
        while (true) {
            cin >> choice;
            if ( choice <= 5 and choice >= 1 ) break;
        ...
        if (choice == 1) {
            ...
            if (!AddPatient(ThePatient.specialization, ThePatient.name, ThePatient.statis , count)) {
                ...
                << "Please enter another Specialization: " << endl << endl;
            }
        }

        else if (choice == 2) PrintAllPatients(count);
        else if (choice == 3) {
            int spec;
            cout << "Enter specialization: " << endl;
            while (true) {
                cin >> spec;
                if (spec >= 1 and spec <= 20) break;
                else cout << "Enter correct specialization to get the next patient: " << endl;
            }
